...
access_patterns=${MAI_BENCH_ACCESS_PATTERNS:-"stream_plain stream_bandwidth policy_stream_pipeline policy_multistream_stride stride_plain sparse_plain random_hotset"}
...
run_access() {
    ...
    if [ "$pattern" = "stream_bandwidth" ] ||
       ...
       [ "$pattern" = "policy_multistream_stride" ]; then
