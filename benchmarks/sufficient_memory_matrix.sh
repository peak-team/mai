...
access_patterns=${MAI_BENCH_ACCESS_PATTERNS:-"stream_plain stream_bandwidth policy_stream_pipeline policy_multistream_stride policy_hotset_scan policy_successor_cycle policy_spatial_region_mask policy_spatial_interleaved_mask stride_plain sparse_plain random_hotset"}
...
run_access() {
    ...
    if [ "$pattern" = "stream_bandwidth" ] ||
       ...
       [ "$pattern" = "policy_successor_cycle" ] ||
       [ "$pattern" = "policy_spatial_region_mask" ] ||
       [ "$pattern" = "policy_spatial_interleaved_mask" ]; then
